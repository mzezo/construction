...
function Gallery() {
  ...
  useEffect(() => {
    if(category){
      ...
      const filteredProjects =  projectsArr?.filter((pro: any) => pro?.attributes?.category?.data?.attributes?.name == category)
                    ...
                    <li onClick={() => setCategory(item?.attributes?.name)}>
                      <button 
                        className={`transition-colors duration-150 uppercase font-semibold tracking-widest 
                          ${category === item?.attributes?.name ? "text-gray-500": "text-black"}`} 
